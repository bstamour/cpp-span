...
constexpr bool container_can_be_used =
    ...
    !detail::is_std_array<Container>::value && !std::is_array_v<Container> &&
...
public:
  ...
  template <std::size_t N, typename = std::enable_if_t<
                               Extent == bst::dynamic_extent || N == Extent>>
  ...
  constexpr span(Container &cont)
      : data_{std::data(cont)}, size_{
                                    static_cast<index_type>(std::size(cont))} {}
...
template <typename T, std::ptrdiff_t Extent>
struct std::tuple_size<bst::span<T, Extent>>
    : std::integral_constant<std::ptrdiff_t, Extent> {};

template <typename T>
struct std::tuple_size<bst::span<T, bst::dynamic_extent>>; // not defined

template <std::size_t I, typename T, std::ptrdiff_t Extent>
struct std::tuple_element<I, bst::span<T, Extent>> {
  using type = T;
};
...
template <std::size_t I, typename T, std::ptrdiff_t Extent,
          typename = std::enable_if_t<Extent != bst::dynamic_extent>>
